...
myhf = init_mol.RHF().run()

PES_driver = myhf.CCSDT()

steps = 300
dt = 10


scanner_fun = PES_driver.nuc_grad_method().as_scanner()
...
myintegrator = md.NVE(
    ...
    incore_anyway=True,
    frames=frames,
    trajectory_output="trajectory_CCSD.xyz",
    data_output="energy_CCSD.xyz",
    verbose=0,
)
myintegrator.run()
...
print(trajectory)
np.save("trajectory_CCSD.npy", trajectory)
