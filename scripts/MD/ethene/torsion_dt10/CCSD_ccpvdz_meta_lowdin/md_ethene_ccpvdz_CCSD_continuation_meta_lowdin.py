...
"""
Runs an MD simulation for an ethene molecule in the cc-pVDZ basis with
CCSD continuation in the meta-Lowdin representation.
"""
...
ccsd_cont = CCSD_EVCont_obj(
    comp_mol=comp_mol,
    nroots=1,
    abstract_basis="meta_lowdin",
)

if os.environ.get("EVCONT_MD_DRY_RUN") == "1":
    print(f"Dry run OK: basis=cc-pvdz, abstract_basis=meta_lowdin, nao={mol.nao}")
else:
    converge_EVCont_MD(
        ccsd_cont,
        init_mol,
