...
def append_to_rdms(mols, overlap=None, one_rdm=None, two_rdm=None):
    ...
    for i in range(len(mols)):
        ...
        orbital_rotation = (
            computational_basis_bra.T.dot(ovlp_bra)
            .dot(oao_basis_bra)
            .dot(computational_to_OAO_ket)
        )
        ...
        ovlp = np.array(
            mps_solver.expectation(
                transformed_bra_state, mps_solver.get_identity_mpo(), ket
            )
        )
        o_RDM = np.array(mps_solver.get_1pdm(ket, bra=transformed_bra_state))
        t_RDM = np.array(
            np.transpose(
                mps_solver.get_2pdm(ket, bra=transformed_bra_state), (0, 3, 1, 2)
            )
        )
