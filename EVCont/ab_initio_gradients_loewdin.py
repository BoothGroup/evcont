...
def get_loewdin_trafo(overlap_mat):
    ...
    inverse_sqrt_vals = jnp.where(vals > 1.e-15, 1/jnp.sqrt(vals), 0.)
...
def get_derivative_ao_mo_trafo(mol):
    ...
    trafo_grad = np.einsum("ijkl, ijmn->klmn", np.array(loewdin_trafo_grad(mol.intor("int1e_ovlp")), dtype=float), overlap_grad)
...
def get_two_el_grad_ao(mol):
    ...
    deriv = deriv + deriv.transpose(0,1,3,2,5,4) + deriv.transpose(0,1,4,5,2,3) + deriv.transpose(0,1,5,4,3,2)
...
def get_two_el_grad(mol, ao_mo_trafo=None, ao_mo_trafo_grad=None):
    ...
    h2_grad = np.einsum("abcd,aimn,bj,ck,dl->ijklmn", h2_ao, ao_mo_trafo_grad, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo, optimize="optimal")

    # This can certainly be done faster via appropriate transpose operations, but it shouldn't be the bottleneck atm
    h2_grad += np.einsum("abcd,ai,bjmn,ck,dl->ijklmn", h2_ao, ao_mo_trafo, ao_mo_trafo_grad, ao_mo_trafo, ao_mo_trafo, optimize="optimal")
    h2_grad += np.einsum("abcd,ai,bj,ckmn,dl->ijklmn", h2_ao, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo_grad, ao_mo_trafo, optimize="optimal")
    h2_grad += np.einsum("abcd,ai,bj,ck,dlmn->ijklmn", h2_ao, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo_grad, optimize="optimal")

    h2_grad += np.einsum("abcdmn,ai,bj,ck,dl->ijklmn", h2_grad_ao, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo, ao_mo_trafo, optimize="optimal")
...
def get_energy_with_grad(mol, one_RDM, two_RDM, S):
    ...
    h1_jac = get_one_el_grad(mol, ao_mo_trafo=ao_mo_trafo, ao_mo_trafo_grad=ao_mo_trafo_grad)

    h2_jac = get_two_el_grad(mol, ao_mo_trafo=ao_mo_trafo, ao_mo_trafo_grad=ao_mo_trafo_grad)

    jac_H = np.sum(np.expand_dims(one_RDM, (-1,-2)) * h1_jac, axis=(-3,-4)) + 0.5 * np.sum(np.expand_dims(two_RDM, (-1,-2)) * h2_jac, axis=(-3,-4,-5,-6))

    en, vec = approximate_ground_state(h1, h2, one_RDM, two_RDM, S)

    return en + mol.energy_nuc(), np.einsum("i,ijkl,j->kl", vec, jac_H, vec) + grad.RHF(scf.RHF(mol)).grad_nuc()
