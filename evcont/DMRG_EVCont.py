...
def append_to_rdms_OAO_basis(
    ...
    OAO basis, allowing for a straightforward contraction of the MPS. This is the
    default, and other options below come with different problems.
    """
...
def append_to_rdms_rerun(
    ...
    different from the OAO basis, by re-running DMRG for each cross term.
    """
    mol_bra = mols[-1]
...
def append_to_rdms_orbital_rotation(
    ...
    """
    Grows the training set by appending to the t-RDMs by running DMRG in a basis
    different from the OAO basis, by approximately rotating the MPS.
    """
    mol_bra = mols[-1]
...
class DMRG_EVCont_obj:
    ...
    def prune_datapoints(self, keep_ids):
        """
        Prunes datapoints based on keep_ids.

        Args:
            keep_ids: The indices of the datapoints to keep.
        """
